...
CREATE TABLE sessions (
    ...
    created_at TIMESTAMP
    WITH
        TIME ZONE DEFAULT NOW(), -- セッション開始日時
        last_activity TIMESTAMP
    WITH
        TIME ZONE DEFAULT NOW() -- 最終活動日時
);

-- 外部キー制約
ALTER TABLE sessions
ADD CONSTRAINT fk_sessions_user_id FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;
...
ADD CONSTRAINT fk_attempts_session_id FOREIGN KEY (session_id) REFERENCES sessions (session_id) ON DELETE CASCADE;

ALTER TABLE attempts
ADD CONSTRAINT fk_attempts_user_id FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;

ALTER TABLE attempts
ADD CONSTRAINT fk_attempts_problem_id FOREIGN KEY (problem_id) REFERENCES problems (problem_id) ON DELETE CASCADE;
...
-- 外部キー制約
ALTER TABLE events
ADD CONSTRAINT fk_events_session_id FOREIGN KEY (session_id) REFERENCES sessions (session_id) ON DELETE CASCADE;

ALTER TABLE events
ADD CONSTRAINT fk_events_user_id FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;

ALTER TABLE events
ADD CONSTRAINT fk_events_attempt_id FOREIGN KEY (attempt_id) REFERENCES attempts (attempt_id) ON DELETE CASCADE;
...
-- users テーブルのRLSポリシー
-- ユーザーは自分のデータのみアクセス可能
CREATE POLICY "Users can access their own data" ON users FOR ALL USING (
    user_id = current_setting ('app.current_user_id', true)
);

-- problems テーブルのRLSポリシー
-- 全ユーザーが問題データを読み取り可能（認証済みユーザーのみ）
CREATE POLICY "Authenticated users can read problems" ON problems FOR
SELECT USING (
...
CREATE POLICY "Users can access their own sessions" ON sessions FOR ALL USING (
    user_id = current_setting ('app.current_user_id', true)
);

-- attempts テーブルのRLSポリシー
-- ユーザーは自分の試行記録のみアクセス可能
CREATE POLICY "Users can access their own attempts" ON attempts FOR ALL USING (
    user_id = current_setting ('app.current_user_id', true)
);

-- events テーブルのRLSポリシー
-- ユーザーは自分のイベントログのみアクセス可能
CREATE POLICY "Users can access their own events" ON events FOR ALL USING (
    user_id = current_setting ('app.current_user_id', true)
);
