...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[debug] Inserting event_logical_symbol:', { kind: eventData.kind, session_id: eventData.session_id, user_id: eventData.user_id, attempt_id: eventData.attempt_id })
    
    const { error: insertError } = await supabase.from('events_logical_symbol').insert(eventData)

    if (insertError) {
      // 重複エラーの場合は成功として扱う（冪等性）
      if (insertError.code === '23505') {
        console.log('[info] Duplicate event_logical_symbol ignored:', idempotencyKey)
        return NextResponse.json({ success: true, message: 'duplicate_ignored' })
      }
      console.error('[error] Event_logical_symbol insert error:', insertError, 'Event data:', eventData)
      return NextResponse.json(
    ...
    console.log('[success] Event_logical_symbol inserted:', eventId)
    ...
    console.error('log-logical-symbol route error:', err)
    return NextResponse.json(
